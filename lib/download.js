...
function downloadTS(tsUrlList, finishList, durationSecond, fileName, id) {
	let isPause = false
	let downloadIndex = 0
	let mediaFileList = []
	let finishNum = 0
	...
	let dealTS = (file, index, callback) => {
		...
		conversionMp4(data, index, (afterData) => {
			...
			chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
				...
				chrome.tabs.sendMessage(tabs[0].id, {action: 'SET', id: `thuthi_progress_${id}`, value: value, color: color});
			});
	...
	let download = () => {
		...
		if (finishList[index] && finishList[index].status === '') {
			ajax({
				...
				type: 'file',
				success: (file) => {
					dealTS(file, index, () => downloadIndex < tsUrlList.length && !isPauseCopy && download())
				},
				fail: () => {
...
async function downloadVideo(url, fileName, id) {
	ajax({
		url: url,
		type:'get',
		success: (m3u8Str) => {
			...
			chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
				chrome.tabs.sendMessage(tabs[0].id, {action: 'SET', id: `thuthi_progress_${id}`, value: '0 / ' + tsUrlList.length, color: "red"});
			});
			if (tsUrlList.length > 0) { 
				downloadTS(tsUrlList, finishList, durationSecond, fileName, id)
			} else {
