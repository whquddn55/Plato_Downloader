...
function downloadTS(tsUrlList, finishList, durationSecond, fileName, className, tabid) {
	let isPause = false
	let downloadIndex = 0
	let mediaFileList = []
	let finishNum = 0
	...
	ajax({
		...
		success: (m3u8Str) => {
			...
			if (tsUrlList.length > 0) { 
				downloadTS(tsUrlList, finishList, durationSecond, fileName, className, tabid)
			} else {
